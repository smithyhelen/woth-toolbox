...
// WOTH coordinates typically range from -2048 to +2048
function gameCoordToPixel(coord: number | undefined, mapSize: number): number {
  if (coord === undefined || coord === null) return 0;
  
  // Convert game coordinate (-2048 to +2048) to pixel (0 to mapSize)
  const COORD_RANGE = 2048;
  const normalized = (coord + COORD_RANGE) / (COORD_RANGE * 2);
...
export function HerdMapOverlay({ currentMap, onHerdsLoaded, mapWidth = 4096, mapHeight = 4096 }: HerdMapOverlayProps) {
  ...
  const toggleHerd = useCallback(async (herdId: number) => {
    setSelectedHerds(prev => {
      const newSelected = new Set(prev);
      
      if (newSelected.has(herdId)) {
        newSelected.delete(herdId);
        setAnimals(animals => animals.filter(a => a.herd_id !== herdId));
      } else {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      const hasToken = typeof window !== 'undefined' && !!localStorage.getItem('jwt_token');
      if (hasToken && (error || !herdsData)) {
        loadHerds();
      }
    }, 1000);

    return () => clearInterval(interval);
  }, [error, herdsData, loadHerds]);
  ...
  return (
    <>
      ...
      <div id="tracked-animals-overlay" style={{
        ...
        {filteredAnimals
          .filter(animal => animal.coordinate_x !== undefined && animal.coordinate_y !== undefined)
          .map((animal, index) => {
            const pixelX = gameCoordToPixel(animal.coordinate_x, mapWidth);
            const pixelY = gameCoordToPixel(animal.coordinate_y, mapHeight);
            
            const tooltipText = [
              `${animal.species_name} - ${'⭐'.repeat(animal.star_rating)} ${animal.age_class}`,
              animal.named_location ? `📍 ${animal.named_location}` : '',
              animal.location_notes ? `Note: ${animal.location_notes}` : '',
              `Caller: ${animal.responds_to_caller ? 'Yes' : 'No'}`,
              `Recommendation: ${animal.culling_recommendation || 'MONITOR'}`
            ].filter(Boolean).join('\n');
            ...
            return (
              <div
                ...
                data-location={animal.named_location || animal.location_notes}
