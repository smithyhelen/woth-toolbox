...
const API_BASE_URL = process.env.NEXT_PUBLIC_API_URL || 'http://217.154.38.113:5021';
...
function DiscordAuthContent() {
  ...
  const loadUserHerds = useCallback(async () => {
    try {
  ...
  const handleJWTToken = useCallback(async (token: string) => {
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined' && !user && !tokenProcessed) {
  ...
  }, [searchParams, tokenProcessed, handleJWTToken, router, user, loadUserHerds]);
      ...
      {!user ? (
        ...
        <div style={{
          ...
          <div style={{ 
            ...
            {user.avatar && (
              <img 
                src={`https://cdn.discordapp.com/avatars/${user.user_id || user.id}/${user.avatar}.png`} 
                alt="avatar"
